...
TEST_FILE = os.path.join(FLAGS.data_dir, 'cifar-10-batches-bin', 'test_batch.bin')
IMAGE_SIZE = 32
...
class Cifar10:
    ...
    def __init__(self):
        ...
        pm = self.compute_per_pixel_mean()
        self.pixel_mean = tf.constant(pm)
        self.scale = tf.constant(1.0 / float(128))
    ...
    def distorted_inputs(self, batch_size):
      ...
      """
        ...
        padding = tf.constant([[4, 4], [4, 4], [0, 0]], dtype=tf.int32)
        padded_image = tf.pad(reshaped_image, padding)
        ...
        float_image = self.scale * (tf.cast(distorted_image, dtype=tf.float32) - self.pixel_mean)
    ...
    def inputs(self, eval_data, batch_size):
      ...
      """
        ...
        # Subtract off the mean and divide by the variance of the pixels.
        float_image = reshaped_image - self.pixel_mean #tf.image.per_image_standardization(resized_image)
        float_image = self.scale * float_image
    ...
    def compute_per_pixel_mean(self):
        x, y = self._load_data(TRAIN_FILES)
        return self._compute_per_pixel_mean(x)

    def load_full_np(self):
        x, y = self._load_data(TRAIN_FILES)
        x_test, y_test = self._load_data([TEST_FILE])
        pixel_mean = self._compute_per_pixel_mean(x)
        x -= pixel_mean
        x /= 128.0
        x_test -= pixel_mean
        x_test /= 128.0
